...
* Enter ```python dumptybreak.py panda.mp4``` to "break apart" a video (in this example, ```panda.mp4```) into its composite frames. The extracted frames will appear in ```.\frames\panda``` (using the name of the input video)
